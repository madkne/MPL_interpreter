...
//-----------------------analyzing source path
    ...
    uint32 size = char_split(source, sep_platform, &entries, true);
    char_split(entries[size - 1], '.', &filename, true);
    String Fsrc = filename[0];
    str_init(&main_source_name, Fsrc);
    project_root = char_join(entries, sep_platform, size - 1, true);
    //printf("GGGGG:%s\n",project_root);
    if (LINUX_PLATFORM == 1) {
        ...
        for (uint32 i = 0; i < line_size; i++) {
            ...
            if (code_line[i] == '\"' && (i == 0 || code_line[i - 1] != '\\')) {
                ...
                if (is_str) {
                    ...
                    append_utst(tmp1);
                    collect = 0;
                    buffer = utf8_str_append(buffer, (str_utf8) str_from_long_int(entry_table.utf8_strings_id));
        ...
        if (!is_multiple_str) {
            ...
            for (uint32 i = 0; i < utf8_str_length(buffer); i++) {
                if (!IS_ONE_BYTE(buffer[i])) {
                    printf("ERR:%i\n", buffer[i]);
                    print_error(line_number, "bad_place_using_utf8", ascii_path, "", "", "open_mpl_file");
                    return false;
                }
