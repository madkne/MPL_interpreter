...
Boolean mpl_help_starter(str_list argvs, uint32 argvs_len) {
  ...
  if (argvs_len == 0) mpl_help_main();
    ...
    sp = char_to_str(OS_SEPARATOR); //=>platform file path separator
    docs_dir = str_multi_append(abs_mpl, sp, "docs", sp, 0, 0); //=>absolute path of 'docs' dir
    String not_exist = 0;
    //---------------------------------------------
    //=>full params array
...
void mpl_help_article(String params[3], String *not_exist) {
  ...
  if (params[1] == 0 && str_search(help_groups, params[0], StrArraySize (help_groups))) {
    if (mpl_help_print_article(str_multi_append(docs_dir, params[0], sp, "index.mdo", 0, 0), "#intro", false) == -1) {
      (*not_exist), params[0];
      return;
    }
    ...
    if (mpl_help_print_article(str_multi_append(docs_dir, params[0], sp, "index.mdo", 0, 0), "#subjects", true) == -1
...
//    printf("subject_path:%s[%i],%s,%s\n", subject_path, index, params[1], print_str_list(list_keys, list_count));
    //=>show article
    if (mpl_help_print_article(str_multi_append(docs_dir, params[0], sp, subject_path, 0, 0), params[2], false) == -1)
      &not_exist, params[1];
    sprintf(footer,
...
void mpl_help_list(uint8 type, String params[3], String *not_exist) {
  ...
  if (type == 0) {
    ...
    if (mpl_help_print_article(str_multi_append(docs_dir, params[0], sp, "index.mdo", 0, 0), "#subjects", false) == -1)
